            ...
            try {
                ...
                switch (pFromServer.getType()) {
                    case ProtocalType.C.FROM_CLIENT_TYPE_OF_COMMON$DATA: {
                        if (ClientCore.getInstance().getChatTransDataEvent() != null) {
                            ClientCore.getInstance().getChatTransDataEvent().onTransBuffer(
                    ...
                    case ProtocalType.C.FROM_CLIENT_TYPE_OF_RECIVED: {
                        ...
                        if (ClientCore.getInstance().getMessageQoSEvent() != null)
                            ClientCore.getInstance().getMessageQoSEvent().messagesBeReceived(theFingerPrint);
...
//						QoS4SendDaemon.getInstance().remove(theFingerPrint);
                    ...
                    case ProtocalType.S.FROM_SERVER_TYPE_OF_RESPONSE$LOGIN: {
                        ...
                        if (loginInfoRes.getCode() == 0) {
                            ClientCore.getInstance().setLoginHasInit(true);
...
//									QoS4SendDaemon.getInstance().stop();
                                    ...
                                    ClientCore.getInstance().setConnectedToServer(false);
//									LocalUDPSocketProvider.getInstance().closeLocalUDPSocket();
                                    ClientCore.getInstance().getChatBaseEvent().onLinkCloseMessage(-1);
                            ...
                            ClientCore.getInstance().setConnectedToServer(true);
                        } else {
                            LocalUDPDataReciever.getInstance().stop();
                            ClientCore.getInstance().setConnectedToServer(false);
                        }

                        if (ClientCore.getInstance().getChatBaseEvent() != null) {
                            ClientCore.getInstance().getChatBaseEvent().onLoginMessage(
                    ...
                    case ProtocalType.S.FROM_SERVER_TYPE_OF_RESPONSE$FOR$ERROR: {
                        ...
                        if (errorRes.getErrorCode() == ErrorCode.ForS.RESPONSE_FOR_UNLOGIN) {
                            ClientCore.getInstance().setLoginHasInit(false);
                        ...
                        if (ClientCore.getInstance().getChatTransDataEvent() != null) {
                            ClientCore.getInstance().getChatTransDataEvent().onErrorResponse(
